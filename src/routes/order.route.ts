...
export function orderRoute(
    ...
    api.get("/order/seller/:sellerId", async (c: Context) => {
        const sellerId = c.req.param("sellerId")
        const { message, orders } = await orderHndlr.getSellerOrders(sellerId)

        return c.json({ message, orders }, StatusCodes.OK)
    })
    api.get("/order/buyer/:buyerId", async (c: Context) => {
        const buyerId = c.req.param("buyerId")
        const { message, orders } = await orderHndlr.getBuyerOrders(buyerId)
